 ...
 */
hpint64
readNumber(const char *mocAscii, int *start)
{
	hpint64		nb = -1;
	...
	/*
	 * If the current character is a digit, read all other possible successive
	 * digits:
	 */
 ...
 */
char
readChar(const char *mocAscii, int *start)
{
	/* Skip all space characters: */
	while (mocAscii[*start] != '\0' && isspace(mocAscii[*start]))
		(*start)++;
...
smoc_eq_impl(Datum a, Datum b)
{
	...
	if (moc_a->order != moc_b->order || moc_a->first != moc_b->first || moc_a->last != moc_b->last || moc_a->area != moc_b->area)
		return false;
	if (moc_a->data_begin != moc_b->data_begin) /* this needs to be
												 * reconsidered if the MOC
												 * version is updated */
		return false;
	...
	if (moc_a_end != moc_b_end) /* this needs to be reconsidered if the MOC
								 * version is updated */
		return false;

	for (int j = moc_a->data_begin; j < moc_a_end; j += MOC_INTERVAL_SIZE)
		/* iterate over both in parallel */
	{
		moc_interval *x;
		moc_interval *y;
...
smoc_overlap_impl(Datum a, Datum b)
{
		...
		/* iterate over both in parallel */
	{
		...
		if (x->second <= y->first)
			/* a entirely left of b, advance a */
		{
			i += MOC_INTERVAL_SIZE;
			continue;
		}
		if (y->second <= x->first)
			/* b entirely left of a, advance b */
		{
			j += MOC_INTERVAL_SIZE;
			continue;
		}
...
smoc_subset_impl(Datum a, Datum b)
{
		...
		/* iterate over a */
	{
		...
		/*
		 * TODO: we could walk the tree structure to find the next interesting
		 * interval
		 */
	}
...
healpix_subset_smoc_impl(hpint64 x, Datum y)
{
	...
	d_end = VARSIZE(moc) - VARHDRSZ;	/* the end of the actually detoasted
										 * part */
...
smoc_gin_extract_internal(Smoc *moc_a, int32 *nkeys, int gin_order)
{
	...
	*nkeys = 0;
	...
	for (int32 a = moc_a->data_begin; a < moc_a_end; a = next_interval(a))
	{
		...
		int			shift = 2 * (HEALPIX_MAX_ORDER - gin_order);	/* degrade to
																	 * MOC_GIN_ORDER */
		...
		{
			if (*nkeys > 0 && keys[*nkeys - 1] == p)
				/* has(larger) pixel already been added ? */
				continue;
			if (*nkeys >= nalloc)
			{
				nalloc *= 2;
